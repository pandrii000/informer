WITH unseen (id) AS (SELECT t1.id FROM repositories t1 WHERE t1.seen = 0),
     last_requests (id_repository, request_datetime) AS (SELECT t2.id_repository, MAX(t2.request_datetime) FROM repositories_requests t2 GROUP BY t2.id_repository)
